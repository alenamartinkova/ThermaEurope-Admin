...
export default function LanguageMenu (): JSX.Element {
  ...
  return (
    <div className="select language-menu items-end flex flex-no-wrap justify-end">
      ...
      <Select.Trigger className="language-menu-trigger bg-white h-10 sm:h-13 w-10 sm:w-13 flex justify-center items-center border-none rounded-25 hover:bg-gray-hover">
        <Select.Icon asChild={true}>
          <img src ="/images/layout/language-select-icon.svg" alt={t?.('layout.lang_select_title')}/>
        </Select.Icon>
      </Select.Trigger>

      <Select.Content className="DropdownMenuContent content bg-white p-2 fixed left-0 sm:left-auto right-0 sm:right-5 top-15 sm:top-15 w-full sm:w-auto sm:rounded-10 py-2.5 px-2 sm:px-2.5 border-t sm:border-0 border-grey-input">
        <Select.Viewport className="w-full sm:w-64 min-w-max">
          <Select.Group>
            <Select.Label className="font-medium leading-5 p-2">{t?.('layout.lang_select_title')}</Select.Label>
